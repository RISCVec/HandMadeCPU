...
class Core extends Module {
    ...
    val if_pc_next  = MuxCase(if_pc_plus4, Seq(
        ...
        (if_inst === ECALL) -> csr_regfile(0x305),  // got to trap_vector
        stall_flg           -> if_reg_pc, // stall
    ))
    if_reg_pc := if_pc_next

    //*************************************
    // IF / ID Register Value Assign
    id_reg_pc := Mux(stall_flg, id_reg_pc, if_reg_pc)
    id_reg_inst := MuxCase(if_inst, Seq(
      // 优先级顺序很重要！跳转成立和停顿同时发生时，优先进行跳转处理
    ...
    // 为了检测stall_flg，临时进队寄存器编号译码
    val id_rs1_addr_b = id_reg_inst(19, 15)
    val id_rs2_addr_b = id_reg_inst(24, 20)

    // 与EX的数据冒险->stall
    val id_rs1_data_hazard = (exe_reg_rf_wen === REN_S) &&
      (id_rs1_addr_b =/= 0.U) && (id_rs1_addr_b === exe_reg_wb_addr)
    val id_rs2_data_hazard = (exe_reg_rf_wen === REN_S) &&
      (id_rs2_addr_b =/= 0.U) && (id_rs2_addr_b === exe_reg_wb_addr)
    stall_flg := (id_rs1_data_hazard || id_rs2_data_hazard)

    // 分支、跳转、停顿时将ID BUBBLE化
    ...
    // OP2_IMI: 2.U(3.W)
    // OP2_IMS: 3.U(3.W)
    val csignals = ListLookup(id_inst, List(ALU_X, OP1_RS1, OP2_RS2, MEN_X, REN_X, WB_X, CSR_X),
        Array(
    ...
    exe_alu_out := MuxCase(0.U(WORD_LEN.W), Seq(
        ...
        (exe_reg_exe_fun === ALU_SRA)   -> (exe_reg_op1_data.asUInt() >> exe_reg_op2_data(4, 0)).asUInt(),
    ...
    val csr_wdata = MuxCase(0.U(WORD_LEN.W), Seq(
        ...
        (mem_reg_csr_cmd === CSR_E) -> 11.U(WORD_LEN.W)     // ECALL in Machine Mode
    ))
